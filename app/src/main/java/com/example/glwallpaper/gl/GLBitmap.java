...
public class GLBitmap {
    ...
    // 顶点坐标数组, 坐标原点为 open gl 的正中心, 向右为 x 正方向, 向上为 y 正方向
    // 可见部分的坐标范围 [-1, -1] - [1, 1], 超出此范围, 不可见
    // 顶点坐标定义 绘制区域, 每个区域是一个三角形, 顶点三角形组合, 有多个方式
    // 下面 定义了两个 三角形,[1] (-1, -1), (1, -1), (-1, 1); [2] (1, -1), (-1, 1), (1, 1)
    // 这个需要动态计算，这里只是展示
    ...
    // 纹理坐标数组, 坐标原点为 纹理的左下角, 向右为 x 正方向, 向上为 y 正方向
    // 纹理坐标 范围 [0, 0] - [1, 1]
